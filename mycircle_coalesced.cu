...
__global__ void count_darts(float* x, unsigned long long* counter, unsigned long long N_darts, int i_repeat, bool verbose)
{
...
int main(int argc, char** argv)
{
    ...
    float* x_host = new float[2 * N_total_darts];
    ...
    for (int i = 0; i < N_repeat; ++i)
    {
        ...
        for (unsigned int j = 0; j < N_darts; ++j)
        {
            ...
            x_host[i * N_darts + j + 1] = distr(gen);
            float dist = sqrt(pow(x_host[i*N_darts+j], 2) + pow(x_host[i*N_darts+j+1], 2));
            if (verbose)
            {
                std::cout <<  " x_host[i*N_darts+j]: " << x_host[i*N_darts+j] <<  std::endl;
                std::cout <<  " x_host[i*N_darts+j+1]: " << x_host[i*N_darts+j+1] <<  std::endl;
    ...
    float* x_device;
    cudaMalloc((void**) &x_device, 2 * N_total_darts * sizeof(float));
    ...
    cudaMemcpy(x_device, x_host, 2 * N_total_darts * sizeof(float), cudaMemcpyHostToDevice);
    count_darts<<<N_block, N_thread_per_block>>>(x_device, counter_device, N_total_darts, 0, verbose);
